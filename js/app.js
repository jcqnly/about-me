...
function q6() { //Q6
  ...
  while (guessForQ6 < 4) {
    guessForQ6++;
    var answer6 = Number(prompt('How many deliveries have I made throughout the lifespan of Futurama? You\'ve got 4 shots.  This will be guess #' + guessForQ6 + '.'));
    if(answer6 === 33) {
    ...
    if(isNaN(answer6)) { //the else if statement style got confusing, so only ifs
      alert('Gotta enter a number, bozo.');
    }
    if(guessForQ6 < 4 && answer6 < 33) {
      alert('Go higher!');
    }
    if(guessForQ6 < 4 && answer6 > 33) {
      alert('Go lower!');
...
function q7(){ //Q7
  ...
  while (guessForQ7 < 6){
    guessForQ7++;
    var answer7 = prompt('What were some of my favorite words? You\'ve got 6 shots.  This will be guess #' + guessForQ7 + '.  Choose from: Fry, bite, Zoidberg, my, Leela, shiny, Farnsworth, metal, Amy, ass, Hermes').toLowerCase().trim();
    ...
    if (answer7 === true) {//boolean break statement from Collins code line 210
      break;
    } else if(guessForQ7 < 6) {
      alert('Guess again!');
    }
  }
...
if(tally > 4){
  ...
  alert('You got ' + tally + ' out of 7 correct!  That\'s terrible!');
}